...
\initTitre{UV \og Projet \fge -- Sujet \ccxf{} \\ Distribution de
  l'architecture OpenStack}
\initAbrevTitre{Projet \ccxf{DIsOStack}}
...
%Elle est aujourd'hui la solution
%  OpenSource retenue et supportée par le plus grand nombre d'acteurs
%  aussi bien académique et qu'industriels (A titre d'exemple HP et
%  RedHat sont les deux companies qui se disputent la premiere place du
%  plus gros contributeur).
  ...
  L'objectif du travail proposer dans ce projet d'option s'insère dans
...
\item la proposition d'une solution et sa mise en \oe uvre au travers
  de prototype experimental (selon l'avancée des travaux réalisées,
  les étudiants pourront mener des campagnes d'experiences sur la
  plateform \href{Grid'5000}{Grid'5000}.
\end{itemize}
