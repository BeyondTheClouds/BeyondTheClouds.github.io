...
%DVMS is an algorithm that targets the dynamic scheduling of VMs hosted on
%arge-scale distributed infrastructure. It  aims at preserving resource
%requirements of VMs   by migrating VMs located on overloaded servers to underloaded ones.
%DVMS works in a cooperative and fully distributed manner.
...
overlay simply provide a link to a potential collaborator. \AL[JP]{How it is structured in the current situation}
...
\AL[JP]{here we do not describe DVMS in general but we should
  emphasize what has been exactly implemented and how}
\JP[AL]{I added the preceding paragraph that gives more details about the overlay
network used.}
...
\AL[JP, MS]{here we miss a paragraph that gives few details regarding
  how the overlay and the communication process have been implemented.}
