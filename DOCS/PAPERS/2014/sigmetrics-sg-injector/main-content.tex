...
collect of resource usage information but simply use an external tool
(namely ganglia), we had different ways to implement the monitoring to
...
can be harnessed to yield a valid simulation: delays induced by
communication in the ``real'' implementation, for instance, can be
easily added as part of the lightweight simulation. We have
implemented this lightweight variant for the monitoring mechanism.

Finally, as mentioned in
Section \ref{subsec:traces-analysis}, we monitor, for each iteration,
