...
Virtual Machine Placement Problem (VMPP) have been developped by the
research community, most of the popular Cloud Computing (CC) system
...
algorithms that have been deeply investigated by our community \cite{feller:ccgrid12},
\cite{Hermenier:2009:ECM:1508293.1508300}, \cite{5715067},
\cite{quesnel:cpe2012}, \cite{5328077}, \cite{5935254} is
...
appropriate solutions for CC production infrastrutures.
% and compare  with existing proposals.
However, \textit{in-vivo} (\ie real-world) experiments,
%
if it is possible to execute them,
%
are always expensive and tedious to perform as it has been once again
highlighted~\cite{barker:pitfalls}. They may even be counterproductive
if the observed behaviours are far from the expected ones due to
unanticipated interferences of different parts of the solution in real
conditions.

\MS{We should include/underline the new programming abstractions:
  \texttt{XVM} etc. } In this article, we propose % the first version of
...
workloads that are specific to the CC paradigm, notably
...
\AL{Fix the organization structure} The rest of the article is
organized as follow. Section~\ref{sec:vmpp} highlights the importance
of the scalability, reliability and reactivity critierions for the VM
