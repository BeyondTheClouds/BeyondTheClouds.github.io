...
simulation framework to perform in-depth investigations of placement
...
demonstrated~\cite{simgridpub} and (ii) because the framework has been
...
\ref{sec:injector} introduces our tool and discusses its general
functioning.
...
amount of CPU/network resources to each \sg abstraction, step by step until the
simulation ends.
...
simulation.
%
The framework has been implemented in JAVA by
...
\subsection{Overview}
At coarse-grained, our framework proceeds a simulation in three
...
is evaluated. Basically, it consists in at least two \sg processes,
...
development of schedulers, our framework is currently released with
...
mechanisms enables us to give concrete examples of
how the deployment of the different \sg processes is done through a
generic script.
...
framework spawns at least two SG processes, the \emph{injector} and the
...
\AL{Future work: add the possibility to have VMs
  appearing/dissapearing on the fly like a dynamic provisioning
  strategies}.
...
available through the \texttt{SimulatorManager} class.
...
Last but not the least, It is noteworthy that our framework really invokes the execution of
