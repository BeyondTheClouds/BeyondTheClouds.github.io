...
from guaranteeing high levels of Service Level Agreement (SLA) and
...
\AL[MS]{Je vois pas ce qui n'est pas bon. Je dis juste que faire des
  expes réels, quand c'est possible c'est d'une part galere et d'autre
part ca peut etre complement inutile si les resultats sont clairement
pas ceux attendus. Ca me parait correcte !?} (\ie real-world) experiments implies,
...
the characteristics of a new proposal and compare it with existing
ones. This way, our approach allows \textit{in vivo} experiments to be
restricted to algorithms that have the potential to handle CC
