...
injection and (iii) visualization. The initialization phase corresponds to
...
algorithm.  Regarding the \emph{injector}, as it name suggest
suggest, it is in charge of injecting the different events that occur
during the execution of the simulations. Current events are VM load
...
%described in Section \ref{sec:vm-schedulers}.
According to the selected algorithm, VMs are relocated through the
different PMs during the execution of the simulation in order to
ensure scheduling objectives and SLA guarantees.
The last phase
consists in analyzing the collected traces in order to generate the
different figures.
...
round-robin manner on the $p$ first PMs defined in the platform file.
...
parameters of each simulation (given in a general java properties file). Although the cluster topology is usually the
...
with real Cloud Computing conditions. Available classes are defined in
a specific text file that can be modified according to the user's needs.
The class of a VM is determined by a random process that selects one
line in the file.
...
events queue that will be consumed during the second phase of the
...
Regarding \emph{CPU load} events, the queue is generated in order to change every $t$
...
Regarding \emph{node crash} events, the queue is generated in order to
...
Once the VMs and the event queue are ready, the evaluation of the
...
\AL[JP]{Could you please write a paragraph explaining the traces
  analysis process.}
