...
rather usual and thus we believe acceptable. Each simulated VM relied
one of the VM classes previously described in Section
\ref{subsec:accuracy}. Ten VMs have been initially launched on each
PM. The parameters for changing the load of the VMs were the same too
($\lambda$ = Nb VMs/300, $\mu$ = 60 and $\sigma$ = 20). The stationary
...
\caption{Simulation - Results Overview 10 VMs per node (VM load: $\mu=60$ and $\sigma=20$)}
...
\begin{center}
  ...
  \end{tabular} }
    ...
    \caption{Mean values and standard deviations (mean $\pm$ standard deviation) of computation duration.}
\label{fig:detailed_computation_time}
\end{center}
\end{figure}

% * {fig:detailed_computation_time}
The conduction of simulations enables the comparison of three different
scheduling algorithms. Firstly, even the implementations of these three
algorithms are all based on Entropy, their respectif amount of nodes invocking
...
\begin{figure}[ht]
\label{fig:detailed_violation_time}
\begin{center}
    {\scriptsize \begin{tabular}{|P{27mm}@{\:}||@{\:}c@{\:}|@{\:}c@{\:}|@{\:}c@{\:}||@{\:}c@{\:}|@{\:}c@{\:}|@{\:}c@{\:}|}
      \thickhline
      \textbf{Violation duration}
        & \multicolumn{3}{c@{\:}||@{\:}}{\textbf{Algorithm}}
          \Tstrut \\
         \hfill  & ~Centralized~ & ~Hierarchical~ & Distributed \Bstrut \\
      \thickhline
  ...
  \end{tabular} }
    ...
    \caption{Mean values and standard deviations (mean $\pm$ standard deviation) of violation duration.}
\end{center}
\end{figure}

% * {fig:experiments_violation_time} and {fig:detailed_violation_time}:
...
\ref{violation_clouds_256} shows how significant is the dispersion of the
violation duration when using the hierarchical algorithm.

\begin{figure}[ht]
\begin{center}
    \includegraphics[width=.95\linewidth]{figures/experiments/256-distributed-hierarchical.pdf}
    \caption{Details of violations duration occuring during simulation (2560 VMS).}
\end{center}
\label{fig:violation_clouds_256}
\end{figure}
...
\begin{center}
    {\scriptsize \begin{tabular}{|P{27mm}@{\:}||@{\:}c@{\:}|@{\:}c@{\:}|@{\:}c@{\:}||@{\:}c@{\:}|@{\:}c@{\:}|@{\:}c@{\:}|}
      \thickhline
      \textbf{Reconfiguration duration}
        & \multicolumn{3}{c@{\:}||@{\:}}{\textbf{Algorithm}}
          \Tstrut \\
         \hfill  & ~Centralized~ & ~Hierarchical~ & Distributed \Bstrut \\
      \thickhline
  ...
  \end{tabular} }
    ...
    \caption{Mean values and standard deviations (mean $\pm$ standard deviation) of reconfiguration duration.}
\end{center}
\label{fig:detailed_violation_time}
\end{figure}
