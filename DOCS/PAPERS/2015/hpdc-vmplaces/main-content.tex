...
\subfigure[Total Reconfiguration Times]{
  \includegraphics[width=.32\linewidth]{figures/groupSizes-reconfigurationTime.pdf}
  \label{fig:groupSizesReconfigTime}}
\subfigure[Total Violation Times]{
  \includegraphics[width=.32\linewidth]{figures/groupSizes-violationTime.pdf}
  \label{fig:groupSizesViolationTime}}
\caption{Experimental results for 4 group sizes and infrastructure sizes}
...
obtained for scenarios with respectively 2,~4,~8 and~32~LCs per GM for
four infrastructure sizes. The table~\ref{fig:groupSizesReconfigFail}
shows how often overloading has been discovered but could not be
resolved because the GM did not manage enough resources. 2~LCs per GM
is clearly insufficient at our load level, failed reconfigurations are
already very rare in the case of 4~LCs per GM and do not occur at all
for 8~and 32~LCs per GM. This is understandable because the load is
statistically evenly distributed among the LCs. Furthermore, the load
profile we evaluated only rarely results in many LCs of a GM to be
overloaded: violations can therefore be resolved even in the case of a
smaller number of 4~LCs available for load distribution.

The overall performance (\ie cumulated violation time), shown if
Fig.~\ref{fig:groupSizesViolationTime} shows that 2~LCs per GM result in
significantly higher violation times: this is due to the relatively
high failure rate. All other group sizes yield violation times that
are relatively close, which seems to indicate that a small group size
does not help much in resolving violations faster. 

However, considering the total times used for reconfigurations, see
Fig.~\ref{fig:groupSizesReconfigTime}, we can see that the duration of
the overall reconfiguration phases has been significantly reduced to
reach a value close to the DVMS ones for the 4-LCs per GM scenario.
We noticed, however, that reducing the number of LCs too significantly
can lead to counterproductive situations if the VM load increases and
GMs do not supervise anymore a sufficient amount of PMs to find
solutions. \MS{Last argument not yet validated: is it obvious enough?}
Once again, this information is valuable as it will help researchers
to design new algorithms favoring the automatic discovery of the
optimal subset of nodes capable to solve a particular
violation. Although DVMS selects naively its neighborhood without
