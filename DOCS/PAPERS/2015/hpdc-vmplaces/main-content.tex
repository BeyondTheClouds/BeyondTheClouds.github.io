...
our analysis of the three algorithms described in the previous Section.
...
In this section, we analyze the results of the simulations we
performed on the Entropy, Snooze and DVMS strategies. After discussing a
rather general comparison, we show the pros and cons of having a
different topology in Snooze. Finally, we present some variants and possible
improvements of Snooze and DVMS that we easily studied thanks to
\vmps.
...
\subsubsection{General  Comparison}
...
To Repair (MTTR) of a server in a Google DC~\cite[pp. 107-108]{datacenterAsComputer}.
\AL[all]{It might make sense to omit that point because this on a 1800
period simulation is a non sense. We should have launched something
with a 160 second crash in average}


\begin{figure}
\subcapcentertrue
\subfigure[Load of the Cluster]{\includegraphics[width=4cm]{./figures/experiments/1024-hierarchical.pdf}\label{fig:load_figure}}
...
256, 512 and 1024 PMs, hosting respectively 1280, 2560, 5120 and 10240 VMs.
Additional simulated PMs have been provided to execute the Entropy and Snooze
service  nodes. For Snooze, one GM has been created every 32 LCs (\ie PMs).
It is noteworthy that no service node had to be provisioned for DVMS as
it had been executed directly on top of the hosting nodes.
...
\ref{table:detailed_computation_time} and \ref{tab:detailed_reconf_time}  give more details by presenting the mean and the
standard deviations of the duration of, respectively, the violations,
the computation phases and the reconfiguration phases.
As anticipated, the centralized approach does not scale well and becomes
even counterproductive in comparison to a system where there is no dynamic
scheduling performed. The more nodes Entropy has to monitor, the less
efficient it is. On the computation phase, Entropy took longer
to resolve the scheduling problem. Moreover, the more nodes are
involved in the reconfiguration the more complex is the
reconfiguration plan leading to some expensive manipulations including
migrations towards the same destination nodes.

\AL[MS]{replay the simulations without any
  algorithm in order to have reference values}
...
\centering
    {\scriptsize \begin{tabular}{|P{27mm}@{\:}||@{\:}c@{\:}|@{\:}c@{\:}|@{\:}c@{\:}||@{\:}c@{\:}|@{\:}c@{\:}|@{\:}c@{\:}|}
      ...
      \thickhline
  \end{tabular} }
\caption{Means $\pm$ Std deviations of computation durations.}
...
\centering
    {\scriptsize \begin{tabular}{|P{27mm}@{\:}||@{\:}c@{\:}|@{\:}c@{\:}|@{\:}c@{\:}||@{\:}c@{\:}|@{\:}c@{\:}|@{\:}c@{\:}|}
      ...
      \thickhline
  \end{tabular} }
\caption{Means $\pm$ Std deviations of reconfiguration durations.}
...
Figure \ref{tab:detailed_computation_time} depicts the
...
the computations on one node, while the distributed and the hierarchical
algorithms will organize computations on several nodes.

This way of organizing computation has a direct impact on performance of the
scheduling policy: according to figure \ref{fig:cumulated_violation}, the
time spend with VMs that are experiencing a QoS violation is dramatically higher than
with the distributed and hierachical scheduler, and this trend continues when
the number of hosting nodes reaches 1024 nodes. Although this validate the
relevance at large scale of distributed scheduling algorithms, it should be
noted that in a carefuly observation Figure \ref{tab:detailed_violation_time}
emphasizes that the results of the distributed policy is more stable than the
hierarchical one, and has better results. In addition to depicting the gap of
performance between the distributed and the hierarchical schedulers, figure
\ref{tab:violation_clouds_snooze_1024} shows how significant is the dispersion
of the violation duration when using the hierarchical algorithm.
...
\subcapcentertrue
\subfigure[DVMS]{
  \includegraphics[width=7cm]{figures/experiments/clouds-1024-distributed.pdf}
  \label{fig:violation_clouds_dvms_1024}}
\subfigure[Snooze]{
  \includegraphics[width=7cm]{figures/experiments/clouds-1024-hierarchical.pdf}
  \label{fig:violation_clouds_snooze_1024}}
\caption{Details of violations duration occuring during simulation (10240 VMS).}
