...
The VMPP can be summarized in three-steps (see Figure
...
Although a recent study on the characterization of production CC
workloads~\cite{birke:nom2014} advocates the use of exponential
...
work. Getting back to the Gaussian, as the CPU load can fluctuate
...
We underline that adding new events can easily be done by simply defining new event Java
...
Finally, we highlight that each random process used in \vmps is initialized with a
seed defined in a configuration file. This way, we can ensure
...
\caption{Details of violations duration occurring during simulation
  (1024 PMs).}
...
%plan to provide a dedicated API to be able to provision
