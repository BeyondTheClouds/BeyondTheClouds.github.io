...
techniques, considering that cooling is accounting for a big part of
...
DCs, alors qu'ils sont à la base de Discovery}
...
mutualize resources, reducing the energy impact while taking the advantage of
delivering UC services as close as possible to the end users. 
...
%To illustrate the advantages of our proposal, 
Figure~\ref{fig:renater} shows a snapshot of the network weather
...
\ftodo[ACO$\rightarrow$AL]{Although multiple small or medium size DCs break the idea of mutualization of resources for energy saving, it is important to...} 
...
appropriate infrastructures, \textit{i.e.}, reliable and efficient enough to operate UC
resources spread across the different PoPs. Ideally, UC resources would be able to
directly
take advantage of computation cycles available on network servers, \textit{i.e.} those
...
network hub with a number of servers dedicated to VMs hosting. 
\ftodo[FQ$\rightarrow$AL]{Network hub == PoP ? Si oui, s/network hub/PoP/g ?}
As we can expect that the
...
many UC resources distributed on distinct sites (in other words, a \emph{LUC
  Operating System}) would enable% in a unified manner,
...
proposal is to consider VMs as the basic objects it manipulates.  Unlike existing
