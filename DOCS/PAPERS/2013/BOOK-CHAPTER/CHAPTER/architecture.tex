...
\ftodo[FQ$\rightarrow$AL]{I merged the two first paragraphs, since they were redundant; for
now, I also removed the reference to micro kernels, since I do not feel comfortable
with the argument that they will make the development and the management easier;
however, we may argue that using micro kernels will make the LUC OS more modular
and extensible; this is more or less the promise of component-oriented programming}
...
we consider enough nodes have been gathered in regards to the requesting
application.

One risk to be mitigated is not to be blocked by a spiral view containing only
...
destination is able to learn long links, which will significantly reduce the
number of hops of future requests. We currently study the amount of requests
needed to approach a logarithmic routing complexity. More generally, we work on
the estimation of the application's activity required to (i) guarantee the
constant efficiency of the overlay, (ii) converge, started from a random
